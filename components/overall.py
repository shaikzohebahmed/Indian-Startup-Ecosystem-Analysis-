...
class PlotLineChart:
    ...
    def __init__(self, temp_df: str, x_axis: str, y_axis: str, layout_title: str):
        fig = px.line(
            temp_df,
            x=x_axis,
            y=y_axis,
...
class Overall:
    ...
    def plot_most_funded_round(self):
        most_funded_type = self.overall_analysis.most_funded_round()

        SubHeader(
            title='Most Funded Round',
            tooltip='Top 10 most funded type of round in startup funding'
        )

        PlotHorizontalBarChart(
            x_axis=most_funded_type['amount'],
            y_axis=most_funded_type['round'],
